...
class Day9Part1 implements Solution {
    day(): number {
        return 9;
    }
    ...
    private calc(h: number[]): number {
        ...
        for (let i = finalArrays.length - 1; i >= 1; i--) {
            ...
            nextArray.push(finalArray[finalArray.length - 1] + nextArray[nextArray.length - 1])
        }
        // console.log('next', finalArrays)
        return finalArrays[0][finalArrays[0].length - 1];
    }
