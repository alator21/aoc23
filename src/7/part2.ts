...
class Day7Part2 implements Solution {
    day(): number {
        return 7;
    }
    ...
    async result(): Promise<string> {
        ...
        // console.log(sortedHands.map(r => HandRank[r.rank()]))
...
class Hand {
    ...
    rank(): HandRank {
        ...
        const containsJoker = keysArray.some(r => r === 'J');
        if (!containsJoker) {
            return this.rankWithoutJoker(keysArray, m);
        }
        const numberOfJokers = m.get('J')!;
        return this.rankWithJoker(keysArray, m, numberOfJokers);
    }

    private rankWithJoker(keysArray: string[], m: Map<string, number>, numberOfJokers: number): HandRank {
    ...
    private rankWithoutJoker(keysArray: string[], m: Map<string, number>): HandRank {
        if (keysArray.length === 1) {
            return HandRank.FIVE_OF_A_KIND;
        }
